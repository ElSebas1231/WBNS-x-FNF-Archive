...
class MainMenuState extends MusicBeatState
{
	...
	override function create() {
		...
		var size:Int = 16;
		bottomText = new FlxText(bottomBG.x + 150, bottomBG.y + 4, FlxG.width, leText, size);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin && canSelectSomething) {
			...
			if (allowMouse || FlxG.mouse.justMoved || FlxG.mouse.justPressed) {
				...
				if (timeNotMoving > 2) Cursor.show();
			}
	...
	function changeItem(huh:Int = 0)
	{
		...
		if (selectedItem != null) {
			if (menuItems.members[curSelected] != null) menuItems.members[curSelected].alpha = (curColumn == MAIN) ? 1 : 0.6;
	...
	function positionMenuItems(intro:Bool) {
		for (i in 0...menuItems.length) {
			...
			menuItem.scale.set((i == curSelected && canSelectSomething) ? 1 : 0.8, (i == curSelected && canSelectSomething) ? 1 : 0.8);
