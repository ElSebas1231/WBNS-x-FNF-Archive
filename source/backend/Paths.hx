...
class Paths
{
	...
	public static function getPath(file:String, ?type:AssetType = TEXT, ?library:Null<String> = null, ?dlcsAllowed:Bool = false):String
	{
		if (dlcsAllowed)
		{
			...
			var modded:String = dlcsFolders(customFile);
	...
	{
		#if sys
		if (FileSystem.exists(dlcsFolders(key)))
			return File.getContent(dlcsFolders(key));
	...
	static public function getAtlas(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		if (OpenFlAssets.exists(myXml) || (FileSystem.exists(myXml))) {
			return FlxAtlasFrames.fromSparrow(imageLoaded, File.getContent(myXml));
		} else {
			var myJson:Dynamic = getPath('images/$key.json', TEXT, library, true);
			if(OpenFlAssets.exists(myJson) || (FileSystem.exists(myJson))) {
	...
	inline static public function getAsepriteAtlas(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		var json:String = dlcsImagesJson(key);
	...
	public static function returnSound(path:Null<String>, key:String, ?library:String) {
		...
		var file:String = dlcsSounds(modLibPath, key);
		if(FileSystem.exists(file)) {
	...
	inline static public function dlcs(key:String = '') {
		return 'dlcs/$key';
	}

	inline static public function dlcsFont(key:String) {
		return dlcsFolders('fonts/' + key);
	}

	inline static public function dlcsJson(key:String) {
		return dlcsFolders('data/' + key + '.json');
	}

	inline static public function dlcsVideo(key:String) {
		return dlcsFolders('videos/' + key + '.' + VIDEO_EXT);
	}

	inline static public function dlcsSounds(path:String, key:String) {
		return dlcsFolders(path + '/' + key + '.' + SOUND_EXT);
	}

	inline static public function dlcsImages(key:String) {
		return dlcsFolders('images/' + key + '.png');
	}

	inline static public function dlcsXml(key:String) {
		return dlcsFolders('images/' + key + '.xml');
	}

	inline static public function dlcsTxt(key:String) {
		return dlcsFolders('images/' + key + '.txt');
	}

	inline static public function dlcsImagesJson(key:String) {
		return dlcsFolders('images/' + key + '.json');
	}

	inline static public function dlcsShader(key:String, ?library:String) {
		return dlcsFolders('shaders/'+key+'.frag');
	}
	
	inline static public function dlcsShaderVertex(key:String, ?library:String) {
		return dlcsFolders('shaders/'+key+'.vert');
	}

	static public function dlcsFolders(key:String) {
		var fileToCheck:String = dlcs('${FreeplaySections.sectionSelected}/$key');
		if(FileSystem.exists(fileToCheck)) {
			return fileToCheck;
		}
		return 'assets/dlcs/$key';
	}
