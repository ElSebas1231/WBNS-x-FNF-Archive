...
{
	public static var STICKER_SET = "stickers-set-wbns";
	...
	public var stickersGone:Bool = false;
	...
	*/
	...
	function regenStickers():Void
	{
		...
		while (xPos <= FlxG.width) {
			...
			var sticky:StickerSprite = new StickerSprite(0, 0, STICKER_SET, sticker);
...
class StickerSprite extends FlxSprite
{
	...
	{
		super(x, y);
		loadGraphic(Paths.image('stickers/' + stickerSet + '/' + stickerName));
		updateHitbox();
...
class StickerInfo
{
	...
	{
		var json = Json.parse(Paths.getTextFromFile('images/stickers/${StickerSubState.STICKER_SET}/stickers.json'));
