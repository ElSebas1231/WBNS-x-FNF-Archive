...
function onCreate()
    ...
    ogMax = getPropertyFromClass('openfl.Lib', 'application.window.maximized')
    ...
    setPropertyFromClass('openfl.Lib', 'application.window.x', 120)
    setPropertyFromClass('openfl.Lib', 'application.window.y', 72)
end
...
function onStepHit()
    ...
    elseif curStep == 1747 then
        runHaxeCode([[
            ...
            game.modchartTweens.set(tag, FlxTween.tween(Lib.application.window, {x: 0, y: 0, width: getVar('winSX'), height: getVar('winSY')-1}, 1.4, {ease: FlxEase.expoIn}));
        ]])
        runTimer('wall',2)
    end
end
...
function onDestroy()
    ...
    setPropertyFromClass('openfl.Lib', 'application.window.maximized', ogMax)
